...
- Admin : **http://localhost:8080/admin** (identifiants = `NUXT_ADMIN_EMAIL` / `NUXT_ADMIN_PASSWORD` du `.env`)
...
- `NUXT_ADMIN_EMAIL` / `NUXT_ADMIN_PASSWORD` / `NUXT_ADMIN_NAME` - compte admin initial (créé uniquement si aucun utilisateur en base)
...
- Rate limiting : 5 tentatives/min sur `/api/auth/login`, 3/min sur `/api/contact` (429 au-delà) - `routeRules` dans [nuxt.config.ts](nuxt.config.ts)
...
- Sessions : cookies scellés/chiffrés (`nuxt-auth-utils`), mot de passe hashé en base
